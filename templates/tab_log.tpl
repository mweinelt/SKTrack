					...
					<td style="vertical-align: top;">
						<table class="shadow" width="710px">
							...
							<tr style="display:{if ($item['raid_end'] == 0)}table-row{else}none{/if};" id="raid{$item['raid_iterator']}_item{$item['item_iterator']}">
								...
								<td style="width:350px"><a href="{$sk['baselink_item']}{$item['item_id']}" target="{$sk['external_link_target']}" class="q{$item['item_quality']}" rel="domain=de,item={$item['item_id']}"><img alt="{$item['item_name']} ({$item['item_id']})" src="http://www.linuxlounge.net/~martin/wowimages/?item={$item['item_id']}" class="item" />&nbsp;{$item['item_name']}</a></td>
